...
<div class="row">
    <div class="col-lg-10 offset-lg-1 col-sm-12">
        <div class="card">
            ...
            <div class="card-body">
                <form id="taskForm" action="{{ route('taskUpdate', $task->id) }}" method="post" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="assign_type" id="assign_role" value="role" {{ $task->assign_type == 'role' ? 'checked' : '' }}>
                                <label class="form-check-label" for="assign_role">Assign by Roles</label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="assign_type" id="assign_custom_users" value="custom" {{ $task->assign_type == 'custom' ? 'checked' : '' }}>
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            <select name="roles[]" class="select2 form-control select2-multiple" multiple="multiple" style="width: 100%;">
                                @foreach ($roles as $role)
                                    <option value="{{ $role->name }}" {{ in_array($role->name, old('roles', $task->roles->pluck('name')->toArray())) ? 'selected' : '' }}>
                                        {{ $role->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="row" id="customUsersSection" style="{{ $task->assign_type == 'custom' ? 'display:block;' : 'display:none;' }}">
                        <div class="col-md-12 mb-3">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <div class="form-check">
                                ...
                                <label class="form-check-label" for="is_extra_no">Обычное поручение</label>
                            </div>
                            @error('type_request')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
